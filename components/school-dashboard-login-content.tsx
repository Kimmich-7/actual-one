...
export default function SchoolDashboardLoginContent() {
  ...
  // If already authenticated, go straight to /admin/school
  useEffect(() => {
    const stored = localStorage.getItem("schoolDashboardUser")
    if (stored) router.replace("/admin/school")
  }, [router])
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true)
      const res = await fetch("/api/auth/school-dashboard/login", {
        method: "POST",
      ...
      // Store separately from Admin Login
      localStorage.setItem("schoolDashboardUser", JSON.stringify({ role: "school_dashboard" }))

      router.replace("/admin/school")
    } catch (err) {
      console.error(err)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 flex flex-col items-center justify-center px-4 py-10">
      {/* Top branding like your other portals */}
